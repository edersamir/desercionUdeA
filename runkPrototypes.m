...
% Initialize values
%m son los atributos y n el numero de objetos
[m n] = size([Xcat Xnum]);
centroids = initial_centroids; %Inicialmente no hay nada
previous_centroids = centroids;
idx = zeros(m, 1); %Matriz de zeros 
previous_idx = idx; 
...
centroids = randomInitCentroids([Xcat Xnum], K); % Inicialización aleatoria de los centroides. 
...
while (j <= max_iters && !converged)
    ...
    [idx, totalCost] = findClosestCentroids(Xnum, Xcat, centroids, ' '); %Ubica los objetos en su cluster mas cercano
    ...
    centroids = computeCentroids(Xnum, Xcat, idx, K); %Actualiza Modas y Means
    j++;
endwhile
...
csvwrite(strcat("costs",num2str(K),".csv"),M);
%csvwrite('costs.csv', M);
%csvwrite('min.csv', [minimumCost, bestRun]);
csvwrite(strcat("min",num2str(K),".csv"),[minimumCost, bestRun]);
